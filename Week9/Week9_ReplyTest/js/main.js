...
function replyCommentAjax(comment_id, reply_user_id, parent_card, comment_text) {
  let xhr = new XMLHttpRequest();
  xhr.open("POST", "func/ajaxmanager.php", true);
  // to use the post method we must set the request headers
  // depending on the form data being sent
...
function outputNewComment(output) {
  ...
  commentsdiv.addEventListener("click", function(e) {
    ...
    if(e.target.classList.contains("delete-post")){
      ...
      let reply_user_id = reply_target.closest(".card-header").firstElementChild.getAttribute("data-comment-user-id");
      ...
      formclone.classList.add("col-md-8");
      ...
      replyCommentAjax(comment_id, reply_user_id, parent_card, comment_text);
    }
